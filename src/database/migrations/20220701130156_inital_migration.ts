...
export async function up(knex: Knex): Promise<void> {
  ...
  await knex.raw("CREATE TABLE IF NOT EXISTS `public`.`goods` ( `id` INT NOT NULL AUTO_INCREMENT, `category_id` INT NOT NULL, `name` VARCHAR(45) NOT NULL, `description` VARCHAR(45) NULL, PRIMARY KEY (`id`), INDEX `category_id_foreign_id` (`category_id` ASC) VISIBLE, CONSTRAINT `category_id_foreign` FOREIGN KEY (`category_id`)  REFERENCES `public`.`category` (`id`) ON DELETE CASCADE ON UPDATE CASCADE)");
}
